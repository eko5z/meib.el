...
(defun meib-love (process message arguments)
...
they love themselves as a group."
  (let* ((channel-name (car (plist-get message :arguments)))
	...
	(let ((names (concat (mapconcat (lambda (name) name) (butlast arguments) ", ") " and " (car (last arguments)))))
	  ...
	  (meib-privmsg process channel-name (format "%s love themselves in... %d%%" names (random 100))))
      (if (not arg2)
	  (progn
	    (random (md5 (downcase arg1)))
	    (meib-privmsg process channel-name (format "%s loves themself in... %d%%" arg1 (random 100))))
	(random (md5 (downcase (concat arg1 arg2))))
	(meib-privmsg process channel-name (format "%s loves %s in... %d%%" arg1 arg2 (random 100)))))))
